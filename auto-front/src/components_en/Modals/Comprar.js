...
function Comprar({ title, token, valorConversaoPublica, valorConversaoSecundario, closeModal }) {
    ...
    return (
        <div className="modal-overlay">
            <div className="modal-content">
                ...
                <h2>{title} (<span className='purple-text'>{token}</span>)</h2>
                <div className="oferta-selector">
                    <button onClick={() => handleTipoOfertaChange('publica')} className={tipoOferta === 'publica' ? 'active' : ''}>PUBLIC OFFER</button>
                    <button onClick={() => handleTipoOfertaChange('secundario')} className={tipoOferta === 'secundario' ? 'active' : ''}>SECONDARY MARKET</button>
                </div>
                ...
                <p>New balance: <span className='purple-text'>{novoSaldo} Tokens {token}</span></p>
                <div className="modal-buttons">
                    <button className="detalhes-button">Details</button>
                    <button className="comprar-button">Buy</button>
                </div>
            </div>
        </div>
    );
